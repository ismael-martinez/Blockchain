...
for v in range(V):
  ...
  for d in d_response:
    ...
    data_cost = random.randint(10, 50)
    deviceId = str(d['deviceId'])
    device_version_id = deviceId + '_' + str(version)
    gradient_hash = sha256('This is going to be the hash of the 1_MB gradient_%s from version %d' % (deviceId, version)).hexdigest()
